      ...
      /include yajl_h
       ...
       // ===============================================================
        ...
        dcl-proc GenerarRequestJson export;
          ...
          callp yajl_addChar('h-timestamp': %trim(vTimestamp));
          callp yajl_endObj();
          // --- HEADER ---
          ...
          callp yajl_beginObj();
          callp yajl_addChar('agentCd': 'HSH');
           callp yajl_addChar('agentTransTypeCode': 'QRYI');
          ...
          callp yajl_addChar('countryCd': 'HND');
          ...
          callp yajl_addChar('agentDt': %trim(fechaHoy));
          ...
          // jsonPtr = yajl_copyBufStr();
          // jsonBuffer = %str(jsonPtr);


                //yajl_saveBuf(vFullFileC: errMsg);
              //jsonBuffer=  yajl_copyBufStr();
        ...
        dcl-proc EnviarPost;
          ...
          fd = open(%trim(vFullFileC): O_RDONLY);
          if fd > 0;
            bytesRead = read(fd: %addr(jsonBuffer): %len(jsonBuffer));
            callp close(fd);
          endif;
