...
namespace QueryBuilder.Builders
{
    ...
    public class InsertQueryBuilder
    {
        ...
        public static InsertQueryBuilder FromType<T>(SqlDialect dialect = SqlDialect.Db2i)
            => new InsertQueryBuilder(typeof(T), dialect);
        ...
        /// <summary>Define la lista de columnas para el INSERT. El orden debe coincidir con los valores que se agreguen.</summary>
        ...
        { _valuesRaw.Add(new List<object>(rawValues.Cast<object>())); return this; }
        ...
        /// <summary>Agrega una fila por posición (parametrizada). El orden debe coincidir con IntoColumns.</summary>
        ...
        public InsertQueryBuilder FromObject(object entity)
        {
            ...
            var targetCols = _columns.Count > 0 ? _columns : cols.Select(c => c.ColumnName).ToList();
...
public enum ParameterType { String, Int, Decimal, Date, DateTime, Bool, Char, Guid }

[AttributeUsage(AttributeTargets.Property, Inherited = false)]
public sealed class ParameterTypeAttribute : Attribute
{
    public ParameterType Type { get; }
    public ParameterTypeAttribute(ParameterType type) => Type = type;
}
...
public sealed class PrecisionAttribute : Attribute
{
    ...
    public PrecisionAttribute(int precision, int scale = 0)
...
/// <summary>
/// Mapea atributos y valores desde DTOs con [TableName], [Library], [ColumnName].
/// </summary>
internal static class ModelInsertMapper
{
    internal sealed class ColumnMeta
    {
        public string ColumnName { get; init; } = "";
        public PropertyInfo Property { get; init; } = default!;
        // Metadatos opcionales (para futura tipificación de parámetros si lo deseas):
        public ParameterType? ParamType { get; init; }
        public int? Size { get; init; }
        public (int precision, int scale)? Precision { get; init; }
    }

    public static (string table, string? library) GetTableAndLibrary(Type t)
    {
        var table = t.GetCustomAttribute<TableNameAttribute>()?.Name
            ?? throw new InvalidOperationException($"El tipo {t.Name} no define [TableName].");

        var library = t.GetCustomAttribute<LibraryAttribute>()?.Name; // opcional
        return (table, library);
    }

    public static IReadOnlyList<ColumnMeta> GetColumns(Type t)
    {
        var metas =
            from p in t.GetProperties(BindingFlags.Public | BindingFlags.Instance)
            let c = p.GetCustomAttribute<ColumnNameAttribute>()
            where c != null
            select new ColumnMeta
            {
                ColumnName = c!.Name,
                Property   = p,
                ParamType  = p.GetCustomAttribute<ParameterTypeAttribute>()?.Type,
                Size       = p.GetCustomAttribute<SizeAttribute>()?.Length,
                Precision  = p.GetCustomAttribute<PrecisionAttribute>() is { } prec
                                ? (prec.Precision, prec.Scale)
                                : (ValueTuple<int, int>?)null
            };

        var list = metas.ToList();
        if (list.Count == 0)
            throw new InvalidOperationException($"El tipo {t.Name} no tiene propiedades con [ColumnName].");

        return list;
    }

    public static object?[] ValuesFromObject(object entity, IReadOnlyList<ColumnMeta> cols)
    {
        var vals = new object?[cols.Count];
        for (int i = 0; i < cols.Count; i++)
            vals[i] = cols[i].Property.GetValue(entity);
        return vals;
    }
}
