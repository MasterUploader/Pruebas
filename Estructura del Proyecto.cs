El código c# esta así actualmente, y como esta funciona, así que tomalo como punto de partida:
...
public class TransaccionesServices(IDatabaseConnection _connection, IHttpContextAccessor _contextAccessor) : ITransaccionesServices
{
    ...
    public async Task<RespuestaGuardarTransaccionesDto> GuardarTransaccionesAsync(GuardarTransaccionesDto guardarTransaccionesDto)
    {
        ...
        var (numeroLote, reservado) = ReservarNumeroLote(perfilTranserver, Convert.ToInt32(yyyyMMdd), "usuario"); //El usuario debe ser YBANET
...
solo aplica las mejoras de código y ten cuidado al actualizar el código, porque hice modificaciones para que este acorde a lo que necesito y a lo que las librerias Restutilities pueden hacer.
