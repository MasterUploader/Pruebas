El codigo Actualmente esta así:
    ...
    : Interfaces.IPaymentAuthorizationService
{
    ...
    public async Task<ResponseAuthorizationManualDto> AuthorizeManualAsync(GetauthorizationManualDto request, CancellationToken ct = default)
    {
        ...
        // 1 Si no es 2xx: mapea a error controlado (tu middleware lo transformará)
        ...
        // 2 Tolerar dos formatos: envelope completo o dto directo
        ...
        // 3 Fallback: no pudimos deserializar → error controlado
        throw new HttpRequestException(
...
Pero cuando no haya respuesta ode un error la consulta no ingresara a ninguno de los try catch, hay que corregirlo
