Tengo el codigo así:
var query = new InsertQueryBuilder(typeof(RSAGE01))
    .FromObject(agencia)
    .Build();


Y me da este error:
Debe llamar primero a IntoColumns(...) antes de Row(...).

El InsertQueryBuilder lo tengo así:
...
public class InsertQueryBuilder(string tableName, string? library = null, SqlDialect dialect = SqlDialect.Db2i)
{
    ...
    public InsertQueryBuilder FromObject(object entity)
    {
        ...
        var targetCols = _columns.Count > 0 ? _columns : [.. cols.Select(c => c.ColumnName)];
