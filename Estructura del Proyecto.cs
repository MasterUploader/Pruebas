Así quedo el código con los nuevos metodos, considera eso como código base y luego aplica las mejoras porque los datos que devuelve ResolverParametrosIntLotes, no se estan pasando hacia PosteoLoteAsync.
    Solo aplica las mejoras, no alteres el código que ya existe y funciona.
...
public class TransaccionesServices(IDatabaseConnection _connection, IHttpContextAccessor _contextAccessor) : ITransaccionesServices
{
    ...
    public async Task<RespuestaGuardarTransaccionesDto> GuardarTransaccionesAsync(GuardarTransaccionesDto guardarTransaccionesDto)
    {
        ...
        var p = ResolverParametrosIntLotes(
            ...
            naturalezaCliente: Convert.ToChar(guardarTransaccionesDto.NaturalezaContable),                 // 'C' o 'D'
        ...
        Task<(int CodigoErrorPosteo, string? DescripcionErrorPosteo, string? nomArc)> respuesta = PosteoLoteAsync(tipoCuenta, Convert.ToDecimal(p.CuentaCliente), Convert.ToDecimal(monto), guardarTransaccionesDto.NaturalezaContable, p.CentroCostoGL, p.Moneda, perfilTranserver, p.Des001, p.Des002, p.Des003);
