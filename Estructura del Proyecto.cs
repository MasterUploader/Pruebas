Quedaria de esta forma las clases:
...
namespace Logging.Services
{
    ...
    public class LoggingService : ILoggingService
    {
        ...
        public string GetCurrentLogFile3()
        {
            try
            {
                ...
                if (context is not null)
                {
                    ...
                    string fullPath = Path.Combine(_logDirectory, context.Request.Path.Value?.Trim('/').Replace("/", "_") ?? "General", fileName);
                ...
                return Path.Combine(_logDirectory, $"Log_{DateTime.UtcNow:yyyyMMdd_HHmmss}.txt");
        ...
        /// se guarde en el mismo archivo. Se organiza por API, controlador, endpoint y fecha.
        /// </summary>
        public string GetCurrentLogFile4()
        {
            try
            {
                var context = _httpContextAccessor.HttpContext;
